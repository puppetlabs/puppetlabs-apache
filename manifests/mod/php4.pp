class apache::mod::php4 (
  $package_name   = undef,
  $package_ensure = 'present',
  $path           = undef,
  $extensions     = ['.php'],
  $content        = undef,
  $template       = 'apache/mod/php4.conf.erb',
  $source         = undef,
  $root_group     = $::apache::params::root_group,
) inherits apache::params {

  if defined(Class['::apache::mod::prefork']) {
    Class['::apache::mod::prefork']->File['php4.conf']
  }
  elsif defined(Class['::apache::mod::itk']) {
    Class['::apache::mod::itk']->File['php4.conf']
  }
  else {
  ...
  if $source and ($content or $template != 'apache/mod/php4.conf.erb') {
    warning('source and content or template parameters are provided. source parameter will be used')
  } elsif $content and $template != 'apache/mod/php4.conf.erb' {
  ...
  Class['::apache::mod::mime'] -> Class['::apache::mod::dir'] -> Class['::apache::mod::php4']

  # Template uses $extensions
  file { 'php4.conf':
    ensure  => file,
    path    => "${::apache::mod_dir}/php4.conf",
    owner   => 'root',
    group   => $root_group,
    mode    => '0644',
